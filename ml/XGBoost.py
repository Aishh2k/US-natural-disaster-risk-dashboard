        ...
        if mask_train.sum() > 10:
            ...
            self.regressor.fit(
                ...
                eval_set=[(X_train[mask_train], y_train[mask_train]), 
                          (X_val[mask_val], y_val[mask_val])], # val0，try-catch
                eval_metric='rmse',
                verbose=False
            )
...
def train_one_target(X, y, y_raw, target_name="Loss"):
    ...
    model = TwoStageXGBoost(
        ...
        regression_params={
            'n_estimators': 500, 'learning_rate': 0.05, 'max_depth': 6,
            'objective': 'reg:squarederror', #  MSE ， Log 
            'n_jobs': -1, 'random_state': 42
        }
    )
    ...
    if target_name == "Loss":
        ...
        #  y_raw ，y  log 
        preds_real = np.expm1(preds) # Fatality  log ，
        y_test_real = y_raw_test
...
def train_and_visualize():
    ...
    # Target 2: Fatality (Log - FatalityLog)
